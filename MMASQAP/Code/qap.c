...
long int evaluation_function_calls = 0; /* times the evaluation function has been used */
long int max_evaluation_function_calls; /* evaluations limit only used when is positive */
...
*/
{
  ...
  return ( ((iterations >= max_iterations) && (elapsed_time( VIRTUAL ) > time_limit)) || 
    ...
    (max_evaluation_function_calls > 0 && evaluation_function_calls >= max_evaluation_function_calls) ); 
...
*/
{
    ...
    fprintf(stderr,"--evaluations  -ev #\t maximum number of evaluations, default -1\n");
...
*/
{
    ...
    max_evaluation_function_calls = -1;
}
